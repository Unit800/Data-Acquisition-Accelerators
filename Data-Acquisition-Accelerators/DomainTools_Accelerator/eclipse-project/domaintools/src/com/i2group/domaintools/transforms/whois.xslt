...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="whoisapi/response">
		<QueryResult>
			...
			<Persons></Persons>
			<Documents>
				<xsl:apply-templates select="whois" />
			</Documents>
			...
			<SubjectOfs>
				<xsl:for-each select="registrant">
					<xsl:variable name="registrant_id" select="generate-id()" />
					<xsl:for-each select="../whois">
						<xsl:call-template name="subject_of">
							<xsl:with-param name="from_id" select="$registrant_id" />
							<xsl:with-param name="to_id" select="generate-id()" />
						</xsl:call-template>
					</xsl:for-each>
				</xsl:for-each>
			...
			<Cards>
				<Card>
					...
					<Document_DocumentDetails>
						...
						<Document_DocumentDescription>Whois Record for <xsl:value-of select="$domain" />.</Document_DocumentDescription>
						<Document_DocumentSource>DomainTools Whois Lookup</Document_DocumentSource>
					</Document_DocumentDetails>
					<Document_AdditionalInformation><xsl:value-of select="record" /></Document_AdditionalInformation>
				</Card>
			</Cards>
		</Document>
	</xsl:template>
	...
	<xsl:template match="registrant">
		<Organization>
			...
			<Cards>
				<Card>
					<xsl:call-template name="cardProvenanceTemplate" />
					<Organization_OrganizationName><xsl:value-of select="current()" /></Organization_OrganizationName>
				</Card>
			</Cards>
		</Organization>
	</xsl:template>
	...
	<xsl:template name="cardProvenanceTemplate">
		<tns:CardProvenance>
			<OriginId Version="1" Type="IAP.DomainTools">
				<Key>
					<String>DomainTools whois API</String>
				</Key>
			</OriginId>
